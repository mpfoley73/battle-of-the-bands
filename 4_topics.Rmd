...
For example, in the plot above, topic, `r topic_lbl[39, ]$topic_long`, mapped highly to only a couple songs: `r lyrics_tidy_2_long %>% filter(song == "Was It All Worth It" & topic_id == "topic_39") %>% pull(topic_weight) %>% scales::percent(accuracy = 1)` for _Was it All Worth It_ and `r lyrics_tidy_2_long %>% filter(song == "Come and Get It" & topic_id == "topic_39") %>% pull(topic_weight) %>% scales::percent(accuracy = 1)`. _We Will Rock You_ was composed primarily of a single topic with a few other contributors.
...
lyrics_tidy_2_long %>% 
  group_by(song_id) %>% 
  summarize(.groups = "drop", max_wt = max(topic_weight)) %>%
  ggplot(aes(x = max_wt)) +
  geom_histogram()

lyrics_tidy_2_long %>% 
  summarize(.groups = "drop", pct_lt10 = sum(topic_weight < .10) / n())
  ggplot(aes(x = topic_weight)) +
  geom_density()
  summarize(gt10 = sum(if_else(topic_weight > .1, 1, 0)))
